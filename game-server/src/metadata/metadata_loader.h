...
struct MailTemplateMeta {
    ...
    uint32_t default_expire_days{0};
...
struct MailConfig {
    ...
    uint32_t max_mail_count{0};

    uint32_t expire_days{0};

    uint32_t claim_all_limit{0};

    uint32_t default_list_limit{0};

    std::vector<MailTemplateMeta> templates;
...
struct WeeklyRankingConfig {

    std::string reset_weekday_kst;

    std::string reset_time_kst;

    std::vector<WeeklyRankingReward> rewards;
...
public:
    ...
    const MailConfig& mail_config() const { return mail_config_; }

    const WeeklyRankingConfig& weekly_ranking_config() const { return weekly_ranking_config_; }
...
private:
    ...
    std::unordered_map<uint32_t, size_t> mail_templates_by_id_;
