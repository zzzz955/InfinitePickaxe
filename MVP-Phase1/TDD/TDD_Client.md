...
## 11. 채굴 존 전환(일일보스/경쟁)
- MiningTab 서브탭: 일반/일일보스/경쟁 구성, 일일보스/경쟁은 리스트형 카드 UI.
- 일일보스/경쟁 카드의 "도전" 시 전용 존으로 이동하고, 전투 중에는 Game 씬의 Topbar/Footer를 숨기고 별도 UI만 노출.
- 존 전환은 Additive 씬 로드/언로드 방식으로 처리하고, 존 종료 시 Additive 씬을 완전히 언로드한 뒤 메인 씬 UI를 복구.
- 존 입장 시 서버 채굴 시뮬레이션 일시 정지, 존 종료 시 즉시 재개(세션 단위).
